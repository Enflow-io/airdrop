...
pub extern "C" fn call() {
    ...
    let receiver: ContractHash = runtime::get_named_arg::<Key>("contract_hash")
    .into_hash()
    .map(|hash| ContractHash::new(hash))
    .unwrap();
    ...
    runtime_args.insert("recipient", receiver).unwrap_or_revert();
    runtime_args.insert("amount", amount).unwrap_or_revert();

    runtime::call_contract::<()>(contract_hash, "transfer", runtime_args);
}